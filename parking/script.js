...
function saveMemberInfo(event) {
    ...
    const keys = ['reg_no', 'owner_name', 'owner_name1', 'email', 'd_o_a', 'd_o_a1', 'slot']
    const obj = {}
...
function getTableData() {
    ...
    const filteredMembers = members.filter(({
        ...
        owner_name1.toLowerCase().includes(searchKeyword.toLowerCase()) ||
        email.toLowerCase().includes(searchKeyword.toLowerCase()) ||
        d_o_a.toLowerCase().includes(searchKeyword.toLowerCase()) ||
        d_o_a1.toLowerCase().includes(searchKeyword.toLowerCase()) ||
...
function insertIntoTableView(item, tableIndex) {
    ...
    const dateOfBirthCell = row.insertCell(5)
    const dateOfExit = row.insertCell(6)
    const slotCell = row.insertCell(7)
    const actionCell = row.insertCell(8)
    idCell.innerHTML = tableIndex
    firstNameCell.innerHTML = item.reg_no
    lastNameCell.innerHTML = item.owner_name
    lastName.innerHTML = item.owner_name1
    emailCell.innerHTML = item.email
    dateOfBirthCell.innerHTML = item.d_o_a
    dateOfExit.innerHTML = item.d_o_a1
...
function showMemberData(id) {
    ...
    $('#show_owner_name1').val(member.owner_name1)
    $('#show_email').val(member.email)
    $('#show_d_o_a').val(member.d_o_a)
    $('#show_d_o_a1').val(member.d_o_a1)
...
function showEditModal(id) {
    ...
    $('#edit_owner_name1').val(member.owner_name1)
    $('#edit_email').val(member.email)
    $('#edit_d_o_a').val(member.d_o_a)
    $('#edit_d_o_a1').val(member.d_o_a1)
...
function updateMemberData() {
    if ($('#edit_reg_no').val() == '' || $('#edit_owner_name').val() == '' || $('#edit_owner_name1').val() == '' || $('#edit_email').val() == '' || $('#edit_d_o_a').val() == '' || $('#edit_d_o_a1').val() == '' || $('#edit_slot').val() == '') {
    ...
    member.owner_name1 = $('#edit_owner_name1').val()
    member.email = $('#edit_email').val()
    member.d_o_a = $('#edit_d_o_a').val()
    member.d_o_a1 = $('#edit_d_o_a1').val()
